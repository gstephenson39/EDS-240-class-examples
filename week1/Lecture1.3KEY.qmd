...
2.  when to define data & map variables globally (e.g. within ggplot()) vs. locally (e.g. within a geom())
...
#### Example 3 (mapping color to continuous variables)
...
-   define data and mappings within geom\_\*() (locally) rather than in ggplot() (globally) -- helpful if you plan to include multiple geoms with different mappings (e.g. you're plotting data from multiple data frames):
...
#### Example 8 (piping into a ggplot)
...
-   `position = "fill"` creates stacked bars of the same height (making it easier to compare proportions):
...
Explore **penguin flipper lengths**. Our goals are to review: 1. more position adjustments and scales 2. updating plot labels 3. faceting
